      ...
      <DialogContent className="max-w-2xl bg-background border-border text-foreground">
        <DialogHeader>
          ...
          <DialogDescription className="flex flex-wrap gap-2 mt-2">
            {society.socCategory.map((cat) => (
              <Badge key={cat} variant="secondary" className="text-xs rounded-full">
                {cat}
              </Badge>
            ))}
          </DialogDescription>
        ...
        <div className="mt-6 space-y-4 text-sm leading-relaxed">
          <p>{society.socAbout}</p>

          {society.socHighlights?.length > 0 && (
            <div>
              <h4 className="font-medium mb-2">Highlights</h4>
              <ul className="list-disc ml-5 space-y-1">
                {society.socHighlights.map((h, idx) => (
                  <li key={idx}>{h}</li>
                ))}
              </ul>
            </div>
          ...
          {society.socKeyEvents?.length > 0 && (
            <div>
              ...
              <ul className="list-disc ml-5 space-y-1">
                {society.socKeyEvents.map((event, idx) => (
                  <li key={idx}>
          ...
          <div className="flex items-center gap-4 mt-4 text-muted-foreground text-sm">
            <span className="flex items-center gap-1">
              ...
              {society.socContact?.team?.length ?? 0} Members
            </span>
            <span className="flex items-center gap-1">
              <Calendar className="w-4 h-4" />
              {society.socKeyEvents?.length ?? 0} Events
            </span>
          </div>

          <div className="mt-4 space-y-1">
            <h4 className="font-medium mb-1">Social Links</h4>
            <ul className="text-blue-600 space-y-1">
              {society.socContact?.socSocials?.instagram !== "_" && (
                <li>
                  <a
                    href={society.socContact.socSocials.instagram}
                    target="_blank"
              ...
              {society.socContact?.socSocials?.linkedin !== "_" && (
                <li>
                  <a
                    href={society.socContact.socSocials.linkedin}
                    target="_blank"
              ...
              {society.socContact?.socSocials?.linktree !== "_" && (
                <li>
                  <a
                    href={society.socContact.socSocials.linktree}
                    target="_blank"
