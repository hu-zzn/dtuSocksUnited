...
export function SocietyCard({
  ...
  return (
    <Card className="h-[50vh] min-h-[45vh] max-h-[65vh] w-full flex flex-col justify-between border border-border bg-card text-foreground transition-all duration-300 group hover:-translate-y-1 hover:shadow-2xl dark:hover:shadow-white/10">
      {/* Header */}
      <CardHeader className="flex-shrink-0 pb-4">
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            <CardTitle className="text-[clamp(1rem,2.2vw,1.2rem)] font-medium group-hover:text-primary transition-colors line-clamp-2">
              {society.socName}
            </CardTitle>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {society.socCategory.slice(0, 2).map((category) => (
                <Badge
                  key={category}
                  variant="secondary"
                  className="text-[clamp(0.7rem,1.8vw,0.85rem)] bg-muted text-foreground hover:bg-accent border-0 rounded-full"
                >
                  {category}
                </Badge>
              ))}
        ...
        <div className="flex flex-col h-full">
          ...
          {society.socHighlights?.length > 0 && (
            <div className="space-y-3">
              <h4 className="font-medium text-[clamp(0.8rem,2vw,0.95rem)]">Highlights</h4>
              <ul className="text-[clamp(0.7rem,1.8vw,0.85rem)] space-y-2 text-muted-foreground overflow-hidden max-h-[10vh]">
