...
export function SocietyGrid() {
  ...
  const categories =
    safeSocieties.length > 0
      ? Array.from(new Set(safeSocieties.flatMap((soc) => soc.socCategory)))
      : [];
  ...
  const filteredSocieties = safeSocieties.filter((society) => {
    ...
    const matchesSearch =
      regex.test(society.socName) ||
      society.socCategory.some((cat) => regex.test(cat)) ||
      (society.socKeyWord ?? []).some((keyword) => regex.test(keyword)) ||
      (society.socKeyEvents ?? []).some((event) => regex.test(event.name));
  ...
  return (
    <div className="space-y-6">
      ...
      {categoryFilter === "all" ? (
        categories.map((category) => {
          const inCategory = filteredSocieties.filter((s) =>
            s.socCategory.includes(category)
          );
          ...
          return (
            <div key={category} className="space-y-4 mb-12">
              <h2 className="text-3xl font-semibold text-primary">{category}</h2>
              <div className="relative">
                <div className="swiper-container">
                  <div className="flex overflow-x-auto gap-6 scrollbar-hide snap-x snap-mandatory">
                    {inCategory.map((society) => (
                      <div
                        key={society._id}
                        className="min-w-[300px] max-w-[320px] snap-start shrink-0"
                      >
